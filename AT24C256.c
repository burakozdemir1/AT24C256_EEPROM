...
void removeData(uint8_t* original_data, int original_length, uint8_t* clean_data)
{
  ...
  int start = 0;
  while (start < original_length && original_data[start] == 0xFF) {
    start++;
  }


  int end = original_length - 1;
  while (end >= 0 && original_data[end] == 0xFF) {
    end--;
  }


  for (int i = start; i <= end; i++) {
    clean_data[clean_length++] = original_data[i];
  }


  clean_data[clean_length] = '\0';
}
